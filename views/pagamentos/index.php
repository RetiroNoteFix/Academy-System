...
<body>
    ...
    <div class="painel" id=painelf>
        <div class="titulo" id="titulof">
            <h1 id="h1titulof">Pagamentos - Pagos</h1>
        </div><!--fim do titulo-->
...
<button  class='btnAumentar' id='btnAumentar'><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M6 12H18M18 12L13 7M18 12L13 17" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg></button></div>
    ...
    <div class="dashboardalunos" id="dashboard">
        <div class="linhasuperiorcadastro">
            ...
            <div class="boxpesquisa3">
            <input id="inputpesquisaproximos" placeholder="Digite o nome do aluno" type="text" name="search" required>
                        <div class="button">
...
<button  class='btnAumentar' id='btnAumentar2'><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M6 12H18M18 12L13 7M18 12L13 17" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg></button></div>
    ...
    <div class="painel" id="paineldesativado">
        <div class="titulo" id="titulof">
        <h1 id="h1titulof">Pagamentos - Ignorados</h1>
        </div><!--fim do titulo-->
    <div class="dashboardalunos" id="dashboard">
        <div class="linhasuperiorcadastro">
            <div class="alinharbtn">
            ...
            <div class="boxpesquisa3">
            <input id="inputpesquisaignorado" placeholder="Digite o nome do aluno" type="text" name="search" required>
                        <div class="button">
                            <button id="buttonpesquisa" type="submit">
                            <svg id='svgiconssearch' fill="#000000" height="200px" width="200px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 490.4 490.4" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M484.1,454.796l-110.5-110.6c29.8-36.3,47.6-82.8,47.6-133.4c0-116.3-94.3-210.6-210.6-210.6S0,94.496,0,210.796 s94.3,210.6,210.6,210.6c50.8,0,97.4-18,133.8-48l110.5,110.5c12.9,11.8,25,4.2,29.2,0C492.5,475.596,492.5,463.096,484.1,454.796z M41.1,210.796c0-93.6,75.9-169.5,169.5-169.5s169.6,75.9,169.6,169.5s-75.9,169.5-169.5,169.5S41.1,304.396,41.1,210.796z"></path> </g> </g></svg>
                            </button>
...
<button  class='btnAumentar' id='btnAumentar3'><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M6 12H18M18 12L13 7M18 12L13 17" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg></button></div>
    ...
    <div id="popuppagamentos" class="popup">
        <div class="closepop"></div>
        <p id="popup-message"></p>
        <div id="dadosaluno"></div>
    </div><!--popuppagamentos-->
    <div id="popuppagamentos2" class="popup">
        <div class="closepop"></div>
...
<script>
    ...
    var optPagamento = document.getElementById("optPagamentoP");
    ...
    var painelProximos = document.getElementById("painelproximos");
...
<script>
    ...
    optHome.addEventListener("mouseover", function() {
        optPagamento.style.backgroundColor = "rgba(34,45,51,255)"
        optPagamento.style.border = "none"
    });
    ...
    optAlunos.addEventListener("mouseout", function() {
        optPagamento.style.backgroundColor = "#0e1114";
        optPagamento.style.borderLeft = "2px solid red"
    });
...
function adicionarEventosBotoes() {
    ...
    linhas.forEach((linha) => {
        const button = linha.querySelector('.verdetalhes');
        ...
        if (button) {
            button.addEventListener('click', function () {
                ...
                const carregarDadosAluno = (ano) => {
                    fetch("../../controllers/FichaPagamentoController.php", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
                        body: JSON.stringify({ id: dados.id, ano: ano }),
                    })
                        .then((response) => {
                            if (!response.ok) {
                            ...
                            if (data.success) {
                                ...
                                if (popupMessage) {
                                    popupMessage.innerHTML = `
                                        <div class='ficha' style='background-color: white; color: black; padding: 20px; border-radius: 10px;'>
                                            <div class='ano-seletor'>
                                                <form id='formanopagamento'>
                                                    <label for='ano' style='color: black;'>Ano:</label>
                                                    <select id='ano' style='color: black; background-color: white; border: 1px solid black; padding: 5px;'>
                                            ...
                                            <h3 style='color: black; text-align: center;'>Ficha de Pagamento - <span id='ano-titulo'>${ano}</span></h3>
                                            <p style='color:black;'><strong style='color: black;'>Nome:</strong> ${data.data.nomeAluno}</p>
                                            <p style='color:black;'><strong style='color: black;'>Endereço:</strong> ${data.data.rua}</p>
                                            <p style='color:black;'><strong style='color: black;'>Telefone:</strong> ${data.data.telefone}</p>
                                            <table class='tabelaPagamento' style='width: 100%; border-collapse: collapse; background-color: white; border: 2px solid #ff9999;'>
                                                <thead>
                                                    <tr style='background-color: #e74c3c; color: white;'>
                                                        <th style='background-color: #e74c3c; padding: 10px; border: 1px solid #e74c3c;'>Mês</th>
                                                        <th style='background-color: #e74c3c; padding: 10px; border: 1px solid #e74c3c;'>Valor</th>
                                                        <th style='background-color: #e74c3c; padding: 10px; border: 1px solid #e74c3c;'>Situação</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                    ...
                                    // Atualiza o total, mas fora da tabela
                                    ...
                                    if (!totalElemento) {
                                        ...
                                        popupMessage.appendChild(novoTotal);
                                    } else {
                                        totalElemento.innerHTML = `<strong>Total: </strong>${total}`;
                                    }

                                    // Adiciona o evento de mudança ao select de ano
                                    const selectAno = document.getElementById('ano');
                                    if (selectAno) {
                                        selectAno.value = ano; // Define o ano selecionado
                                        selectAno.addEventListener('change', function () {
                                            const novoAno = this.value;
                                            carregarDadosAluno(novoAno);
                                        });
                                    }
                                }
                            } else {
                                const popupMessage = document.getElementById("popup-message");
                                if (popupMessage) {
                                    popupMessage.innerHTML = `<p>${data.message || "Dados do aluno não encontrados."}</p>`;
                                }
                            }

                            // Exibe o popup e o overlay
                            const popupPagamentos = document.getElementById("popuppagamentos");
                            const overlay = document.getElementById("overlay");
                            if (popupPagamentos && overlay) {
                                popupPagamentos.classList.add("show");
                        ...
                        .catch((error) => {
                            ...
                            const popupMessage = document.getElementById("popup-message");
                            if (popupMessage) {
                                popupMessage.innerHTML = `<p>Erro ao carregar os dados do aluno.</p>`;
                            }
                ...
                // Carrega os dados do aluno com o ano inicial (2025)
                carregarDadosAluno(2025);
            });
        }
    });
}

// Função para montar a tabela de mensalidades
function montarTabelaMensalidades(mensalidades) {
    ...
    mensalidades.forEach((mensalidade) => {
        const mes = new Date(mensalidade.dataPagamento).getMonth();
            ...
            pagamentos.forEach((pagamento) => {
                tabelaHTML += `
                    <tr>
                        ...
                        <td class="status ${pagamento.situacao.toLowerCase()}">${pagamento.situacao}</td>
                    </tr>
                `;
            });
        } else {
...
// Garante que o script seja executado após o DOM estar carregado
...
<script>
 function adicionarEventosBotoesFuturos() {
    const linhas = document.querySelectorAll('.linha-aluno');

    linhas.forEach((linha) => {
        const button = linha.querySelector('.verdetalhesfuturos');
...
const vencimentoFormatado = formatarData(vencimento);
                ...
                fetch("../../controllers/FichaController.php", {
                    ...
                    .then((data) => {
                        ...
                        const popupPagamentos = document.getElementById("popuppagamentos2");
                        const overlay = document.getElementById("overlay");

                        // Verifica se os elementos do popup existem
                        if (!popupMessage || !popupPagamentos || !overlay) {
                        ...
                        if (data.success && data.data && data.data.id) {
                            ...
                            popupMessage.innerHTML = `
                                <div class="boxpagamento">
                                    <form id="formpagamentos">
                                        <p style='color: #000;'>Nome: ${data.data.nomeAluno} <br><span id='alert'>PAGAMENTO REFERENTE A DATA DE:</span>
                                            <u style='color: #000;'>${vencimentoFormatado}</u>
                                            <input type="hidden" name="idpagarnotificado" value="${data.data.id}">
                                            <input type="hidden" name="datavencimentonotificada" value="${vencimento}">
                                        </p>
                                        <table border="1" style="width: 200px; border-collapse: collapse; text-align: left; margin: 0 auto; font-size: 14px;">
                                            <thead>
                                                <tr>
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td style="padding: 5px;">
                                                        <input required name="valornotificado" style="padding: 5px; height: 25px;" id="input-valor" placeholder="R$0,00" type="text">
                                                    </td>
                                                    <td style=" display:none; padding: 5px;">
                                                        <input  style="display: none;" required name="data_pagamentonotificada" value="${vencimento}" style="padding: 5px; height: 25px;" type="text">
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        <div class="boxbotoes" style="display: flex; width: 100%; height: 100%; flex-direction: row;">
                                            <button type="submit" id="btnpagar" class="cadastro" style="width: 30%; border: none; transition: 0.6s ease; cursor: pointer; background-color: #00aeff;">
                                                
                                                <p style="color: #fff; margin-top: 20px;">Enviar Pagamento</p>
                                            </button>
                                            <button type="button" id="btnfechar" class="cadastro" onclick="closepopuppagamentos()">
                                                <div class="icon">
                            ...
                            if (formPagamentos) {
                                formPagamentos.addEventListener('submit', function (event) {
...
atualizaProximos(1);
                                        ...
                                        .catch((error) => {
                                            console.error("Erro:", error);
                                            popupMessage.innerHTML = `<p>Erro na comunicação com o servidor.</p>`;
                                            setTimeout(() => {
                                                closepopuppagamentos();
                                            }, 1800);
                                        });
...
// Garante que o script seja executado após o DOM estar carregado
document.addEventListener('DOMContentLoaded', adicionarEventosBotoesFuturos);

</script>

<script>
     function adicionarEventosBotoesIgnorados() {
    const linhas = document.querySelectorAll('.linha-aluno');

    linhas.forEach((linha) => {
        const button = linha.querySelector('.verdetalhesignorados');
...
// Garante que o script seja executado após o DOM estar carregado
document.addEventListener('DOMContentLoaded', adicionarEventosBotoesIgnorados);

</script>
<script>
    // Referências ao DOM
    const inputPesquisa2 = document.getElementById("inputpesquisa");
    const resultados2 = document.getElementById("resultadopagamentos");
    const resultado2 = document.getElementById("resultadopagamentos2");
    var painelDesativado = document.getElementById("paineldesativado");
    var painelPrincipal = document.getElementById("painelf");
    ...
    const termoPesquisa2 = inputPesquisa2.value.trim().toLowerCase();

    if (termoPesquisa2 === "") {
        // Exibe resultadospagamentos e oculta resultadospagamentos2
    ...
    resultados2.classList.add("hidden");
    resultado2.classList.remove("hidden");

    // Cria o objeto de dados para enviar com a requisição POST
    const data2 = new FormData();
    data2.append('pesquisapago', termoPesquisa2); // Passa o termo de pesquisa
...
<script>
    // Referências ao DOM
    const inputPesquisa3 = document.getElementById("inputpesquisaproximos");
    const resultados3 = document.getElementById("resultadopagamentosproximos");
    const resultado3 = document.getElementById("resultadopagamentosproximos2");
    var painelDesativado1 = document.getElementById("paineldesativado");
    var painelPrincipal1 = document.getElementById("painelf");
    var tabelaResultado1 = document.getElementById("tabelaresultado");
    var btnAtivos1 = document.getElementById("btnativos");

    inputPesquisa3.addEventListener("input", () => {
        const termoPesquisa3 = inputPesquisa3.value.trim().toLowerCase();

        if (termoPesquisa3 === "") {
            // Quando o campo de pesquisa estiver vazio:
            resultados3.classList.remove("hidden"); // Exibe o primeiro tbody (resultados3)
            resultado3.classList.add("hidden"); // Esconde o segundo tbody (resultado3)
            return;
        }

        // Quando há algo no campo de pesquisa:
        resultados3.classList.add("hidden"); // Esconde o primeiro tbody (resultados3)
        resultado3.classList.remove("hidden"); // Exibe o segundo tbody (resultado3)

        // Cria o objeto de dados para enviar com a requisição POST
        const data3 = new FormData();
        data3.append('pesquisafuturo', termoPesquisa3); // Passa o termo de pesquisa
        ...
        .then(response => response.text()) // Espera a resposta como texto (HTML)
        .then(html => {
            resultado3.innerHTML = html; // Atualiza o conteúdo da tabela com os resultados da pesquisa
        })
        .catch(error => {
            resultado3.innerHTML = "<p>Ocorreu um erro ao carregar os dados.</p>"; // Exibe uma mensagem de erro, se necessário
        });
    });
</script>
...
<script>
    // Referências ao DOM
    const inputPesquisa4 = document.getElementById("inputpesquisaignorado");
    const resultados4 = document.getElementById("resultadopagamentosignorados");
    const resultado4 = document.getElementById("resultadopagamentosignorados2");
    var painelDesativado2 = document.getElementById("paineldesativado");
    var painelPrincipal2 = document.getElementById("painelf");
    var tabelaResultado2 = document.getElementById("tabelaresultado");
    var btnAtivos2 = document.getElementById("btnativos");

    inputPesquisa4.addEventListener("input", () => {
        const termoPesquisa4 = inputPesquisa4.value.trim().toLowerCase();

        if (termoPesquisa4 === "") {
            // Quando o campo de pesquisa estiver vazio:
            resultados4.classList.remove("hidden"); // Exibe o primeiro tbody (resultados3)
            resultado4.classList.add("hidden"); // Esconde o segundo tbody (resultado3)
            return;
        }

        // Quando há algo no campo de pesquisa:
        resultados4.classList.add("hidden"); // Esconde o primeiro tbody (resultados3)
        resultado4.classList.remove("hidden"); // Exibe o segundo tbody (resultado3)

        // Cria o objeto de dados para enviar com a requisição POST
        const data4 = new FormData();
        data4.append('pesquisaignorado', termoPesquisa4); // Passa o termo de pesquisa
        ...
        .then(response => response.text()) // Espera a resposta como texto (HTML)
        .then(html => {
            resultado4.innerHTML = html; // Atualiza o conteúdo da tabela com os resultados da pesquisa
        })
        .catch(error => {
